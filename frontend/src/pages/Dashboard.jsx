...
const Dashboard = () => {
    ...
    const [newHabitWeeklyQuota, setNewHabitWeeklyQuota] = useState(3);
    ...
    const [noteModal, setNoteModal] = useState({ open: false, habitId: null, dateStr: null, note: '' });
    ...
    useEffect(() => {
        api.getPushSettings().then(res => {
            ...
            setPushTime(res.reminderTime || '20:00');
        }).catch(err => console.log('Push settings not loaded', err));
    }, []);

    const handleEnablePush = async () => {
        try {
            if (!('serviceWorker' in navigator) || !('PushManager' in window)) {
                return setNotification("Push Notifications not supported by your browser");
            }
            const permission = await Notification.requestPermission();
            if (permission === 'denied') {
                setPushModalOpen(false);
                return setNotification("Permission blocked. Please click the site icon in your URL bar and 'Allow' notifications.");
            } else if (permission !== 'granted') {
                setPushModalOpen(false);
                return setNotification("Notification permission not granted.");
            }
            ...
            await api.subscribePush(subscription, pushTime);
            setPushEnabled(true);
            setNotification("Push Notifications Enabled! Test notification sent.");
            setPushModalOpen(false);
        } catch (err) {
            console.error("Push Error:", err);
            setNotification(`Failed: ${err.message || "Could not subscribe"}`);
        }
    };
    ...
    const gridCols = `280px ` + dayNumbers.map((_, i) => `${i > 0 && i % 7 === 0 ? '16px ' : ''}36px`).join(' ') + ` 140px`;
    ...
    const handleAddHabit = async () => {
        ...
        try {
            const habit = await api.createHabit({
                ...
                weeklyQuota: newHabitFrequency === 'weekly_quota' ? newHabitWeeklyQuota : 0
            });
    ...
    const handleToggleDate = async (habitId, day) => {
        ...
        setHabits(prev => prev.map(h => {
            if (h._id === habitId) {
                const newDates = h.completedDates.includes(dateStr)
        ...
        try {
            ...
            setNotification("Failed to save progress. Reverting...");
        }
    };

    const handleOpenNote = (e, habitId, dateStr, currentNote) => {
        e.preventDefault(); // Stop default context menu
        setNoteModal({ open: true, habitId, dateStr, note: currentNote || '' });
    };

    const handleSaveNote = async () => {
        try {
            const updatedHabit = await api.addHabitNote(noteModal.habitId, noteModal.dateStr, noteModal.note);
            setHabits(prev => prev.map(h => h._id === noteModal.habitId ? updatedHabit : h));
            setNoteModal({ open: false, habitId: null, dateStr: null, note: '' });
            setNotification("Context note saved!");
        } catch (e) {
            console.error(e);
        ...
        return (
            <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    <div style={{ width: '40px', height: '40px', border: '4px solid var(--primary-color)', borderTopColor: 'transparent', borderRadius: '50%', animation: 'spin 1s linear infinite', margin: '0 auto 1rem' }} />
                    <h2 style={{ color: 'var(--primary-color)', fontWeight: '800' }}>Synchronizing...</h2>
                </div>
        ...
        <div style={{ minHeight: '100vh', background: '#050505', color: '#fff' }}>
            ...
            <main style={{ maxWidth: '1400px', margin: '0 auto', padding: '1.5rem' }}>
                <div className="animate-fade-in">
                    ...
                    <div style={{ background: 'rgba(13, 13, 15, 0.5)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '1.5rem', padding: '2rem', marginBottom: '2rem' }}>
                        <div className="mobile-stack" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', gap: '1.5rem' }}>
                            <div className="mobile-text-center dashboard-header-flex" style={{ display: 'flex', alignItems: 'center', gap: '15px', flexWrap: 'wrap', width: '100%' }}>
                                <div style={{ textAlign: 'inherit' }}>
                                    <h1 style={{ fontSize: '2rem', fontWeight: '900', margin: 0 }}>{currentMonthName} <span style={{ color: 'var(--primary-color)' }}>{currentYear}</span></h1>
                                    <p style={{ color: '#555', fontWeight: '700', marginTop: '4px', fontSize: '0.8rem', letterSpacing: '1px' }}>DISCIPLINE OVER MOTIVATION</p>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                                    <button onClick={() => setPushModalOpen(true)} style={{ background: pushEnabled ? 'var(--primary-gradient)' : 'rgba(255,255,255,0.05)', color: pushEnabled ? '#000' : '#fff', border: 'none', padding: '10px', borderRadius: '50%', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                        🔔
                                    </button>
                                    {user?.role === 'demo' && (
                                        <button
                                            onClick={() => navigate('/request-admin')}
                                            style={{ background: 'transparent', color: 'var(--primary-color)', border: '1px solid rgba(0, 204, 255, 0.4)', padding: '8px 16px', borderRadius: '12px', cursor: 'pointer', fontWeight: 900, fontSize: '0.8rem', whiteSpace: 'nowrap' }}
                                        >
                                            Request Admin
                                        </button>
                                    )}
                        ...
                        <div style={{ marginBottom: '2.5rem', background: 'rgba(0,0,0,0.2)', padding: '0.8rem', borderRadius: '18px', border: '1px solid rgba(255,255,255,0.03)' }}>
                            <div className="mobile-stack" style={{ display: 'flex', gap: '1rem' }}>
                                <div style={{ display: 'flex', flex: 1, gap: '10px', width: '100%' }}>
                                    <input
                                        type="text"
                                    ...
                                    <button
                                        ...
                                        style={{
                                            background: showOptions ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.03)',
                                            color: '#fff', padding: '0 1.2rem', borderRadius: '12px', border: 'none',
                                            fontWeight: '900', cursor: 'pointer', transition: '0.2s', flexShrink: 0
                                        }}>⚙️</button>
                                </div>
                                <button
                            ...
                            {showOptions && (
                                <div className="animate-fade-in" style={{ marginTop: '1rem', paddingTop: '1rem', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                        <label style={{ fontSize: '0.8rem', fontWeight: 800, color: '#aaa', minWidth: '100px' }}>FREQUENCY:</label>
                                        <select
                                            value={newHabitFrequency}
                                            onChange={(e) => setNewHabitFrequency(e.target.value)}
                                            style={{ background: '#111', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '0.5rem 1rem', borderRadius: '8px', fontSize: '0.85rem', outline: 'none' }}
                                        >
                                    ...
                                    {newHabitFrequency === 'specific_days' && (
                                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                            <label style={{ fontSize: '0.8rem', fontWeight: 800, color: '#aaa', minWidth: '100px' }}>DAYS:</label>
                                                ...
                                                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((dayChar, i) => (
                                                    <button
                                                        ...
                                                        style={{
                                                            ...
                                                            background: newHabitSpecificDays.includes(i) ? 'var(--primary-gradient)' : 'rgba(255,255,255,0.05)',
                                                            color: newHabitSpecificDays.includes(i) ? '#000' : '#888',
                                    ...
                                    {newHabitFrequency === 'weekly_quota' && (
                                        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                            <label style={{ fontSize: '0.8rem', fontWeight: 800, color: '#aaa', minWidth: '100px' }}>TIMES/WEEK:</label>
                                            <div style={{ display: 'flex', alignItems: 'center', background: '#111', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '8px', overflow: 'hidden' }}>
                                                <button
                                                    onClick={() => setNewHabitWeeklyQuota(prev => Math.max(1, prev - 1))}
                                                    style={{ background: 'rgba(255,255,255,0.02)', border: 'none', color: '#fff', padding: '0.6rem 0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' }}
                                                ...
                                                <div style={{ color: 'var(--primary-color)', padding: '0.5rem 1rem', fontSize: '0.9rem', fontWeight: '900', textAlign: 'center', minWidth: '40px', borderLeft: '1px solid rgba(255,255,255,0.1)', borderRight: '1px solid rgba(255,255,255,0.1)' }}>
                                                    {newHabitWeeklyQuota}
                                                </div>
                                                <button
                                                    onClick={() => setNewHabitWeeklyQuota(prev => Math.min(7, prev + 1))}
                                                    style={{ background: 'rgba(255,255,255,0.02)', border: 'none', color: '#fff', padding: '0.6rem 0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: '0.2s' }}
                                                    onMouseOver={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.05)'}
                                                    onMouseOut={(e) => e.currentTarget.style.background = 'rgba(255,255,255,0.02)'}
                                                >
                        ...
                        <div style={{ overflowX: 'auto', borderRadius: '2rem', border: '1px solid rgba(255, 255, 255, 0.05)', background: '#080809', marginBottom: '2rem', width: '100%', WebkitOverflowScrolling: 'touch' }}>
                            <div style={{ minWidth: 'fit-content', position: 'relative' }}>
                                {/* Premium Vertical dark line */}
                                <div style={{ position: 'absolute', left: '280px', top: '20px', bottom: '20px', width: '2px', background: 'linear-gradient(to bottom, transparent, rgba(255, 255, 255, 0.15), transparent)', opacity: 0.8, zIndex: 2, pointerEvents: 'none' }}></div>
                                <div style={{
                                    display: 'grid',
                                    ...
                                    {dayNumbers.map((d, i) => (
                                        <React.Fragment key={d}>
                                            {i > 0 && i % 7 === 0 && <div />}
                                            <div style={{ textAlign: 'center', fontSize: '0.75rem', fontWeight: '900', color: '#333' }}>{d}</div>
                                        </React.Fragment>
                                    ))}
                                ...
                                {habits.length > 0 ? (
                                    habits.map(habit => {
                                        ...
                                        let expected = daysInMonth;
                                        if (habit.frequencyType === 'specific_days') {
                                            let count = 0;
                                            for (let k = 1; k <= daysInMonth; k++) {
                                                if (habit.specificDays?.includes(new Date(currentYear, currentMonthIdx, k).getDay())) count++;
                                            }
                                            expected = count || 1;
                                        } else if (habit.frequencyType === 'weekly_quota') {
                                            let weeks = {};
                                            for (let d = 1; d <= daysInMonth; d++) {
                                                const date = new Date(currentYear, currentMonthIdx, d);
                                                const weekKey = date.getDate() - date.getDay();
                                                if (!weeks[weekKey]) weeks[weekKey] = { possible: 0, completed: 0 };
                                            ...
                                            for (let key in weeks) {
                                                const w = weeks[key];
                                                const target = Math.min(w.possible, habit.weeklyQuota || 3);
                                                expected += Math.max(target, w.completed);
                                            }
                                            if (expected === 0) expected = 1;
                                        }

                                        const prog = Math.min(100, Math.round((comp / expected) * 100));
                                        return (
                                            <div key={habit._id} style={{
                                                ...
                                                <div style={{ paddingLeft: '1.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                                    <div style={{ flex: 1, minWidth: 0 }}>
                                                        {editingHabitId === habit._id ? (
                                                            <input autoFocus value={editTitle} onChange={(e) => setEditTitle(e.target.value)} onBlur={() => handleUpdateTitle(habit._id)} onKeyDown={(e) => e.key === 'Enter' && handleUpdateTitle(habit._id)} style={{ background: '#000', border: '1px solid var(--primary-color)', color: '#fff', padding: '6px 12px', borderRadius: '10px', width: '90%', outline: 'none', fontSize: '0.9rem' }} />
                                                        ) : (
                                                            <div style={{ overflow: 'hidden' }}>
                                                                ...
                                                                <div style={{ fontSize: '0.7rem', color: '#ff8a00', marginTop: '4px', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
                                                                    <span style={{ filter: 'grayscale(0)' }}>🔥</span> Streak {comp || 0}
                                                                    {habit.frequencyType === 'specific_days' && <span style={{ color: '#555', background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '4px', marginLeft: '6px' }}>Specific Days</span>}
                                                                    {habit.frequencyType === 'weekly_quota' && <span style={{ color: '#555', background: 'rgba(255,255,255,0.05)', padding: '2px 6px', borderRadius: '4px', marginLeft: '6px' }}>{habit.weeklyQuota || 3} / week</span>}
                                                                </div>
                                                            </div>
                                                        )}
                                                    </div>
                                                ...
                                                {dayNumbers.map((day, i) => {
                                                    ...
                                                    if (habit.frequencyType === 'specific_days' && !habit.specificDays?.includes(tDow)) {
                                                        isActionable = false;
                                                    }
                                                            ...
                                                            <div style={{ display: 'flex', justifyContent: 'center' }}>
                                                                {isActionable ? (
                                                                    <div
                                                                        ...
                                                                        onContextMenu={(e) => handleOpenNote(e, habit._id, dStr, habit.notes?.[dStr])}
                                                                        onTouchStart={(e) => {
                                                                            window.touchTimer = setTimeout(() => handleOpenNote(e, habit._id, dStr, habit.notes?.[dStr]), 500);
                                                                        }}
                                                                        onTouchEnd={() => clearTimeout(window.touchTimer)}
                                                                        onTouchMove={() => clearTimeout(window.touchTimer)}
                                                                        style={{
                                                                            position: 'relative',
                                                                            width: '24px',
                                                                            height: '24px',
                                                                            ...
                                                                            background: isC ? 'var(--primary-gradient)' : 'rgba(255,255,255,0.02)',
                                                                    ...
                                                                    >
                                                                        {isC && '✓'}
                                                                        {habit.notes && habit.notes[dStr] && (
                                                                            <div style={{ position: 'absolute', top: '-4px', right: '-4px', width: '8px', height: '8px', background: '#ff8a00', borderRadius: '50%', border: '2px solid #080809' }}></div>
                                                                        )}
                                                                    </div>
                                                                ) : (
                                                                    <div style={{ width: '24px', height: '24px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#222', fontSize: '0.75rem', fontWeight: 900 }}>-</div>
                                                                )}
                                                            </div>
                                                        </React.Fragment>
                                                    );
                                                ...
                                                <div style={{ textAlign: 'center' }}>
                                                    <div style={{ width: '80px', height: '4px', background: 'rgba(255,255,255,0.02)', borderRadius: '10px', margin: '0 auto 8px', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.03)' }}>
                                                        <div style={{ width: `${prog}%`, height: '100%', background: 'var(--primary-gradient)', borderRadius: '10px' }}></div>
                                                    </div>
                                                    <span style={{ fontSize: '1rem', fontWeight: '900', color: 'var(--primary-color)', letterSpacing: '-0.5px' }}>{prog}%</span>
                                                </div>
                                            </div>
                                        );
                                    })
            ...
            {pushModalOpen && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
                    <div style={{ background: '#111', padding: '2rem', borderRadius: '24px', maxWidth: '400px', width: '90%', border: '1px solid rgba(255,255,255,0.1)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <h2 style={{ fontSize: '1.5rem', fontWeight: '900', margin: 0 }}>Web Push <span style={{ color: 'var(--primary-color)' }}>Reminders</span></h2>
                            <button onClick={() => setPushModalOpen(false)} style={{ background: 'transparent', color: '#888', border: 'none', fontSize: '1.2rem', cursor: 'pointer' }}>✕</button>
                        </div>
                        <p style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
                            Install this app or allow push notifications to receive subtle nudges for incomplete habits at your specified time.
                        </p>

                        <div style={{ marginBottom: '2rem' }}>
                            <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: 800, color: '#666', marginBottom: '0.5rem' }}>REMINDER TIME</label>
                            <input
                                type="time"
                                value={pushTime}
                                onChange={(e) => setPushTime(e.target.value)}
                                style={{
                                    width: '100%', background: '#050505', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '1rem', borderRadius: '12px', fontSize: '1.1rem', outline: 'none'
                                }}
                            />
                        </div>
                        ...
                        <div style={{ display: 'flex', gap: '10px' }}>
                            <button
                                ...
                                style={{ flex: 1, padding: '1rem', borderRadius: '12px', border: 'none', background: 'var(--primary-gradient)', color: '#000', fontWeight: '900', cursor: 'pointer' }}
                            >
                                {pushEnabled ? 'UPDATE REMINDER' : 'ENABLE NOTIFICATIONS'}
                            </button>
                            {pushEnabled && (
                                <button
                                    onClick={handleDisablePush}
                                    style={{ padding: '1rem', borderRadius: '12px', border: '1px solid rgba(255,100,100,0.3)', background: 'rgba(255,100,100,0.1)', color: '#ff6666', fontWeight: '900', cursor: 'pointer' }}
                                >
                                    DISABLE
                                </button>
                            )}
            ...
            {/* Micro-Journal Note Modal */}
            {noteModal.open && (
                <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(5px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
                    <div style={{ background: '#111', padding: '2rem', borderRadius: '24px', maxWidth: '400px', width: '90%', border: '1px solid rgba(255,255,255,0.1)' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                            <h2 style={{ fontSize: '1.5rem', fontWeight: '900', margin: 0 }}>Daily <span style={{ color: 'var(--primary-color)' }}>Context</span></h2>
                            <button onClick={() => setNoteModal({ open: false, habitId: null, dateStr: null, note: '' })} style={{ background: 'transparent', color: '#888', border: 'none', fontSize: '1.2rem', cursor: 'pointer' }}>✕</button>
                        </div>
                        <p style={{ color: '#aaa', fontSize: '0.9rem', marginBottom: '1.5rem', lineHeight: 1.5 }}>
                            Write a brief note for <strong>{noteModal.dateStr}</strong>. How did it feel? Any obstacles?
                        </p>
                        ...
                        <div style={{ marginBottom: '2rem' }}>
                            <textarea
                                ...
                                placeholder="E.g., Felt super sluggish today, but pushed through anyway."
                                style={{
                                    width: '100%', background: '#050505', color: '#fff', border: '1px solid rgba(255,255,255,0.1)', padding: '1rem', borderRadius: '12px', fontSize: '0.9rem', outline: 'none', minHeight: '120px', resize: 'vertical'
                                }}
                            />
                        </div>
                        ...
                        <button
                            ...
                            style={{ width: '100%', padding: '1rem', borderRadius: '12px', border: 'none', background: 'var(--primary-gradient)', color: '#000', fontWeight: '900', cursor: 'pointer' }}
                        >
                            SAVE NOTE
                        </button>
                    </div>
...
const PerformanceInsights = React.memo(({ habits, currentMonthIdx, currentYear }) => {
    ...
    const monthlyProgressData = React.useMemo(() => habits.map(h => {
        ...
        if (h.frequencyType === 'specific_days') {
            let count = 0;
            for (let k = 1; k <= daysInMonth; k++) {
                if (h.specificDays?.includes(new Date(currentYear, currentMonthIdx, k).getDay())) count++;
            }
            expected = count || 1;
        } else if (h.frequencyType === 'weekly_quota') {
            let weeks = {};
            for (let d = 1; d <= daysInMonth; d++) {
                const date = new Date(currentYear, currentMonthIdx, d);
                const weekKey = date.getDate() - date.getDay();
            ...
            for (let key in weeks) {
                const w = weeks[key];
                const target = Math.min(w.possible, h.weeklyQuota || 3);
                expected += Math.max(target, w.completed);
            }
            if (expected === 0) expected = 1;
        }
        ...
        return {
            ...
            fullName: h.title,
            progress: Math.min(100, Math.round((compCount / expected) * 100))
        };
    ...
    return (
        <div style={{ marginBottom: '4rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '2.5rem', paddingLeft: '0.5rem' }}>
                <div style={{ padding: '12px', background: 'rgba(var(--primary-rgb), 0.1)', borderRadius: '16px', border: '1px solid rgba(var(--primary-rgb), 0.2)' }}>
                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="var(--primary-color)" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20V10"></path><path d="M18 20V4"></path><path d="M6 20v-4"></path></svg>
                </div>
                <h2 style={{ fontSize: '1.8rem', fontWeight: '900', color: '#fff', margin: 0, letterSpacing: '-1px' }}>Performance <span style={{ color: 'var(--primary-color)', textShadow: '0 0 20px rgba(var(--primary-rgb), 0.3)' }}>Insights</span></h2>
            </div>
                        ...
                        <BarChart data={monthlyProgressData} margin={{ left: 0, right: 10 }}>
                            <defs>
                                <linearGradient id="barBlue" x1="0" y1="0" x2="0" y2="1">
                                    ...
                                    <stop offset="100%" stopColor="var(--primary-second-color)" stopOpacity={1} />
                                </linearGradient>
                            </defs>
                ...
                <InsightCard title="CONSISTENCY">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.65rem', color: '#555', fontWeight: 800 }}>
                            <span style={{ width: 7, height: 7, background: 'var(--primary-color)', borderRadius: '50%' }}></span> Consistent
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '0.65rem', color: '#555', fontWeight: 800 }}>
            ...
            <div className="mobile-stack" style={{ display: 'grid', gridTemplateColumns: '1fr 1.3fr 1fr', gap: '1.2rem' }}>
                ...
                <SummaryCard title="💡 Recommended Action" color="var(--primary-color)">
                ...
                <SummaryCard title="⭐ Top Consistent" color="#ffd700">
                    {[...monthlyProgressData].sort((a, b) => b.progress - a.progress).slice(0, 5).map((d, i) => (
                        <div key={i} style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px', fontSize: '0.85rem' }}>
                            <span style={{ fontWeight: 800, color: '#aaa' }}>{d.fullName}</span>
                            <span style={{ color: 'var(--primary-color)', fontWeight: 900 }}>{d.progress}%</span>
                        </div>
                    ))}
                </SummaryCard>
            </div>
...
const InsightCard = ({ title, children }) => (
    ...
    >
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '1.5rem' }}>
            <div style={{ width: '4px', height: '14px', background: 'var(--primary-color)', borderRadius: '4px', boxShadow: '0 0 10px rgba(var(--primary-rgb), 0.5)' }}></div>
        ...
        <style>{`
            .insight-card-premium:hover {
                ...
                box-shadow: inset 0 0 20px rgba(var(--primary-rgb), 0.05), 0 30px 60px rgba(0, 0, 0, 0.6);
            }
        `}</style>
    </div>
);
    ...
    if (active && payload && payload.length) {
        return (
            <div style={{
                ...
                <p style={{ margin: 0, fontSize: '0.9rem', fontWeight: 900, color: 'var(--primary-color)' }}>{payload[0].value}{unit}</p>
            </div>
        );
    }
    return null;
