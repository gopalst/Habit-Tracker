...
const History = () => {
    ...
    const activeHabitsForPeriod = habits.filter(h => {
        ...
        // Hide if deleted before or during this month
        if (h.isDeleted) {
            if (h.deletedAt) {
                const deleteDate = new Date(h.deletedAt);
                if (deleteDate <= periodEnd) return false;
            } else {
                return false; // Fallback if missing deletion date
            }
        }
    ...
    const getExpectedDays = (habit, year, monthIdx, maximumDays) => {
        let expected = maximumDays;
        if (habit.frequencyType === 'specific_days') {
            let count = 0;
            for (let k = 1; k <= maximumDays; k++) {
                if (habit.specificDays?.includes(new Date(year, monthIdx, k).getDay())) count++;
            }
            expected = count || 1;
        } else if (habit.frequencyType === 'weekly_quota') {
            let weeks = {};
            for (let d = 1; d <= maximumDays; d++) {
    ...
    const handleExportPDF = () => {
        try {
            ...
            const totalsRow = ['TOTAL', ...activeHabitsForPeriod.map(h => {
                ...
                const expected = getExpectedDays(h, selectedYear, selectedMonth, daysInMonth);
            ...
            activeHabitsForPeriod.forEach((habit) => {
                if (currentYForChart + barHeight > pageHeight - 15) {
                    doc.addPage();
                    doc.setFontSize(14); doc.setTextColor(10, 10, 12);
                    doc.text('MONTHLY PERFORMANCE CHART (CONT.)', 14, 20);
                    currentYForChart = 30;
                }

                const mPre = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}`;
                const count = (habit.completedDates || []).filter(d => d.startsWith(mPre)).length;
                const expected = getExpectedDays(habit, selectedYear, selectedMonth, daysInMonth);
                const pct = Math.min(100, Math.round((count / expected) * 100));

                doc.setFontSize(8); doc.setTextColor(80);
                let title = habit.title.toUpperCase();
                if (title.length > 25) {
                    title = title.substring(0, 22) + '...';
                }
                doc.text(title, 14, currentYForChart + 6);
                doc.setFillColor(240, 240, 240); doc.rect(50, currentYForChart, chartWidth * 0.8, barHeight, 'F');
                doc.setFillColor(0, 204, 255); doc.rect(50, currentYForChart, (chartWidth * 0.8) * (pct / 100), barHeight, 'F');
                doc.setTextColor(0, 102, 128); doc.setFontSize(7);
                doc.text(`${pct}%`, 50 + (chartWidth * 0.8) + 5, currentYForChart + 6);

                currentYForChart += (barHeight + barSpacing);
            });
        ...
        return (
            <div style={{ minHeight: '100vh', background: '#050505', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ textAlign: 'center' }}>
                    ...
                    <h2 style={{ color: 'var(--primary-color)', fontWeight: '800' }}>Loading History...</h2>
                </div>
                ...
                <div style={{ background: 'rgba(13, 13, 15, 0.5)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '2rem', padding: '2rem' }}>
                    <header className="mobile-stack" style={{ marginBottom: '3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                        <div className="mobile-text-center">
                            <h1 style={{ fontSize: '2.5rem', fontWeight: '900', margin: 0 }}>
                                Habit <span style={{ color: 'var(--primary-color)' }}>Ledger</span>
                            </h1>
                        ...
                        <div className="mobile-stack" style={{ display: 'flex', gap: '30px', alignItems: 'flex-end' }}>
                            ...
                            <div className="mobile-stack" style={{ display: 'flex', gap: '15px', justifyContent: 'center', alignItems: 'center', width: '100%' }}>
                                <button onClick={handleShowActivity} style={{ background: 'var(--primary-color)', color: '#000', border: 'none', padding: '10px 25px', borderRadius: '12px', fontWeight: '900', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', height: '43px' }}>Show Activity</button>
                                <button onClick={handleExportPDF} style={{ background: 'transparent', color: 'var(--primary-color)', border: '1px solid rgba(var(--primary-rgb), 0.3)', padding: '10px 20px', borderRadius: '12px', fontWeight: '900', fontSize: '0.8rem', textTransform: 'uppercase', letterSpacing: '1px', cursor: 'pointer', height: '43px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    {activeHabitsForPeriod.length === 0 ? (
                        <div style={{ textAlign: 'center', padding: '8rem 2rem', background: 'rgba(255,255,255,0.01)', borderRadius: '2rem', border: '1px dashed rgba(255,255,255,0.05)' }}>
                            ...
                            <p style={{ maxWidth: '500px', margin: '0 auto 2.5rem', lineHeight: '1.8', color: 'var(--primary-color)', fontSize: '1.1rem', fontStyle: 'italic', fontWeight: '600' }}>"{quotes[Math.floor(Math.random() * quotes.length)]}"</p>
                            <Link to="/dashboard" style={{ textDecoration: 'none', background: 'var(--primary-color)', color: '#000', padding: '1rem 2.5rem', borderRadius: '15px', fontWeight: '900', textTransform: 'uppercase', letterSpacing: '1px', fontSize: '0.85rem' }}>Begin New Journey</Link>
                        </div>
                    ) : (
                        <div style={{ overflowX: 'auto', borderRadius: '1.5rem', border: '1px solid rgba(255, 255, 255, 0.05)', background: '#080809' }}>
                            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                                <thead>
                                    <tr style={{ background: 'rgba(255,255,255,0.02)', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                        ...
                                        {activeHabitsForPeriod.map(habit => (
                                            <th key={habit._id} style={{ padding: '1.5rem 1.5rem', color: 'var(--primary-color)', fontSize: '0.8rem', fontWeight: '900', letterSpacing: '1px', textTransform: 'uppercase' }}>
                                                {habit.title}
                                                    ...
                                                    return (
                                                        <td key={habit._id} style={{ padding: '1.2rem 1.5rem' }}>
                                                            {isBeforeCreation ? <span style={{ color: '#222', fontWeight: '900', fontSize: '1rem' }}>-</span> : (
                                                                <span style={{ padding: '6px 16px', borderRadius: '50px', fontSize: '0.75rem', fontWeight: '900', textTransform: 'uppercase', background: isCompleted ? 'rgba(var(--primary-rgb), 0.1)' : 'rgba(239, 68, 68, 0.05)', color: isCompleted ? 'var(--primary-color)' : '#444', border: isCompleted ? '1px solid rgba(var(--primary-rgb), 0.2)' : '1px solid rgba(239, 68, 68, 0.1)' }}>
                                                                    {isCompleted ? 'YES' : 'NO'}
                                                                </span>
                                                            )}
                                                        </td>
                                ...
                                <tfoot>
                                    <tr style={{ background: 'rgba(var(--primary-rgb), 0.03)', borderTop: '2px solid rgba(255,255,255,0.05)' }}>
                                        <td style={{ padding: '1.5rem 2.5rem', fontWeight: '900', color: 'var(--primary-color)', fontSize: '0.8rem', letterSpacing: '1px' }}>TOTAL</td>
                                        {activeHabitsForPeriod.map(habit => {
                                            const mPre = `${selectedYear}-${String(selectedMonth + 1).padStart(2, '0')}`;
                                            const compCount = (habit.completedDates || []).filter(d => d.startsWith(mPre)).length;
                                            const expected = getExpectedDays(habit, selectedYear, selectedMonth, daysInMonth);
                                            const percentage = Math.min(100, Math.round((compCount / expected) * 100));
                                            return (
                                                <td key={habit._id} style={{ padding: '1.5rem 1.5rem' }}>
                                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                        <span style={{ color: '#fff', fontWeight: '900', fontSize: '1rem' }}>{compCount}/{expected}</span>
                                                        <span style={{ color: 'var(--primary-color)', fontWeight: '800', fontSize: '0.7rem', opacity: 0.8 }}>({percentage}%)</span>
                                                    </div>
                                                </td>
                                            );
                                        })}
